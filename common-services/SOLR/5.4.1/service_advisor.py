...
class SOLR541ServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def validate_solr_ssl_configuration(self):
        items = [
            ...
            self.validator_entry('solr.ssl.key.store.password', self.is_not_null_or_empty, self.solr_ssl_properties),
            self.validator_entry('solr.ssl.trust.store', self.is_absolute_path, self.solr_ssl_properties),
            self.validator_entry('solr.ssl.trust.store.password', self.is_not_null_or_empty, self.solr_ssl_properties),
    ...
    def validate_example_collection_configuration(self):
        items = [
            self.validator_entry('solr.collection.sample.create', self.is_boolean, self.example_collection_properties),
            ...
            self.validator_entry('solr.collection.sample.config.directory', self.is_not_null_or_empty,
                                 self.example_collection_properties),
            self.validator_entry('solr.collection.sample.shards', self.is_number, self.example_collection_properties),
            self.validator_entry('solr.collection.sample.replicas', self.is_number, self.example_collection_properties)
        ]
